...
def parse_args(argv=None):
    ...
    mdl.add_argument("--batch-size", type=int, default=1,
                     help="GPU inference batch size; start with 1 for long "
...
def preprocess_stay(task):
    ...
    sid, g = task
    lines = serialize_lines(g, _WORKER_LABELS_MAP)
    if not lines:
        return sid, None
...
def score_mortality_batch(model, processor, yes_id, no_id, user_texts):
    """Score a batch using Yes/No logits at the answer position."""
    import torch
    texts = []
    ...
    with torch.inference_mode():
        # Avoid allocating vocabulary logits for every input position.
        logits = model(**inputs, logits_to_keep=1).logits[:, -1, :]
    pair = torch.stack([logits[:, yes_id], logits[:, no_id]], dim=1)
    probabilities = torch.softmax(pair.float(), dim=1)[:, 0].cpu().tolist()
    lengths = inputs["attention_mask"].sum(dim=1).cpu().tolist()
    return probabilities, lengths
...
def main(argv=None):
    ...
    with ProcessPoolExecutor(
        ...
        for i, (sid, data) in enumerate(
                executor.map(preprocess_stay, tasks, chunksize=4), 1):
            if data is not None:
                stay_data[sid] = data
    ...
    header_tokens = len(tokenizer(
        FILLER_HEADER, add_special_tokens=False).input_ids)

    sids = sorted(stay_data)
    # Iterate by condition so each GPU batch contains similarly sized inputs,
    # reducing padding and VRAM waste. build_filler() is deterministic, so the
    # before/after variants still receive exactly the same filler text.
    def iter_requests():
        for name, b, placement in conditions:
            for sid in sids:
                d = stay_data[sid]
                record = (
                    "PATIENT RECORD (this patient, chronological, most recent "
                    "last):\n" + "\n".join(d["base_lines"])
                )
                if placement is None:
                    body = record
                else:
                    filler = build_filler(sid, b - d["base_tokens"])
                    if placement == "after":
                        body = record + "\n\n" + filler
                    else:
                        body = filler + "\n\n" + record
                ...
                yield sid, name, d["label"], d["base_tokens"], user_text

    rows = []
    total_requests = len(sids) * len(conditions)
